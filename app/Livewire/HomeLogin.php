    ...
    public function searchLomba(){
        if($this->search_lomba != '' || strlen($this->search_lomba) > 5){
            ...
            if($lomba_res->count() <= 0){
                $this->dispatch('toast:error',message:'Tidak ada lomba yang ditemukan dengan kata kunci '.$this->search_lomba);
                return;
            }
            $this->list_lomba = $lomba_res;
            $this->dispatch('toast:success',message:'Menampilkan hasil pencarian dengan kata kunci '.$this->search_lomba);
            return;
        }
        $this->search_lomba = '';
    }
